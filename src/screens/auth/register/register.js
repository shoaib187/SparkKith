...
export default function Register({ navigation, route }) {
  ...
  const handleRegister = async () => {
    ...
    try {
      if (activeItem?.image) {
        const imageString = getImageAsString(activeItem.image);
        if (imageString) {
          formData.append('profilePicture', imageString);
        }
    ...
    console.log('FormData ready:', {
      ...
      email,
      password,
      profilePicture: activeItem?.image ? getImageAsString(activeItem.image) : activeItem?.imageUrl
    });
